<template>
	<view class="container">
		...
		<view class="list-cell m-t b-b" @click="navTo('清除缓存')" hover-class="cell-hover" :hover-stay-time="50">
			<text class="cell-tit">{{ $t('settings.cache') }}</text>
			<text class="cell-more yticon icon-you"></text>
		</view>
		...
		<view class="list-cell log-out-btn" @click="toLogout">
			<text class="cell-tit">{{ $t('settings.logout') }}</text>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onShow() {
			// 设置页面标题
			setPageTitle('pageTitle.settings');
		},
		methods:{
			...mapMutations(['logout']),
			...
			toLogout(){
				uni.showModal({
				    content: this.$t('auth.logoutConfirm'),
				    success: (e)=>{
				    	if(e.confirm){
				    		this.logout();
				    		setTimeout(()=>{
